...
export function buildAntiguedadCarteraQuery(fechaCorte: string, idEmpresa: number): string {
  ...
  return `DECLARE @FechaCorte DATE = '${safeFechaCorte}';
DECLARE @IdEmpresa  INT  = ${idEmpresa};

WITH MovimientosSaldo AS
(
    /* =========================
...
(
    SELECT
        ...
        ,CASE X.Estatus
            WHEN 4 THEN
                CASE
                    WHEN CONVERT(DATE, DATEADD(DAY, 1, @FechaCorte)) > X.Cancelacion THEN 0
                    ELSE X.TotalFactura
                END
            ELSE X.TotalFactura
    ...
    (
        ...
        WHERE CONVERT(DATE, Z.dFecha) < CONVERT(DATE, DATEADD(DAY, 1, @FechaCorte))
...
(
    ...
    WHERE SUC.nIdEmp11 = @IdEmpresa
      AND CG.nStatus <> 1
      AND CG.dFactura < DATEADD(DAY, 1, @FechaCorte)
),
Base_NC AS
(
    SELECT
        ...
        ,CASE
            ...
            ELSE C.sRFC
         END AS RFC
        ,DATEDIFF(DAY, NC.dNota, @FechaCorte) AS DiasTranscurridos
        ,CASE
    ...
    WHERE SUC.nIdEmp11 = @IdEmpresa
      AND NC.dNota < DATEADD(DAY, 1, @FechaCorte)
      AND
      (
          CASE
              WHEN ISNULL(DNC.nIdCtaGastos15, 0) = 0 THEN ISNULL(DNC.nImporteME, 0)
